...
C/C++ 数组允许你定义变量，来联合一些相同类型的数据项，但结构（**structure**）是另一种用户定义的数据类型，允许你将不同类型的数据项放在一起。   

结构用来表示一条记录，假设你想跟踪在图书馆里的书。您可能想要跟踪每本书的以下属性：
...
**structure tag** 是可选的，每个成员的定义都是一个正常的变量定义，如 int i；或者 float f；或者任何其他有效的变量定义。在结构的定义结束时，在最后的分号（";"）这个符号之前可以指定一个或多个结构变量，但它是可选的。这是声明书结构体的方式:
